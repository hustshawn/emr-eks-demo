...
locals {
  ...
  amg_description            = "Amazon Managed Grafana workspace for ${local.name}"
  account_id                 = data.aws_caller_identity.current.account_id
  amg_name                   = "amp-${local.name}"
}
...
module "managed_grafana" {
  ...
  description               = local.amg_description
  account_access_type       = "CURRENT_ACCOUNT"
  authentication_providers  = ["AWS_SSO"]
  permission_type           = "SERVICE_MANAGED"
  data_sources              = ["CLOUDWATCH", "PROMETHEUS", "XRAY"]
  notification_destinations = ["SNS"]
  stack_set_name            = local.amg_name
  ...
  iam_role_name                  = local.amg_name
  use_iam_role_name_prefix       = true
  iam_role_description           = local.amg_description
  ...
  depends_on = [module.amp_ingest_irsa]
}
